...
export default function AuthScreen() {
  ...
  const handleSocialAuth = async (method: 'google' | 'apple') => {
    ...
    setIsLoading(true);
    try {
      // TODO: Integrate with actual Google/Apple OAuth providers
      // Step 1: Get OAuth token from provider
      // Step 2: Call backend /api/auth/social endpoint with token
      // Step 3: Get user profile from API response
      ...
      const message = error instanceof ApiError ? error.message : `Failed to authenticate with ${method}`;
  ...
  const handleEmailLogin = async () => {
    ...
    try {
      ...
      // TODO: Parse response and set up user profile
      setMode('signup');
    } catch (error) {
      const message = error instanceof ApiError ? error.message : 'Failed to sign in. Please try again.';
  ...
  const handleSendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      // TODO: Connect to phone verification API
      // const response = await api.sendPhoneOtp({ phone: phone.trim() });
      setShowOtp(true);
    } catch (error) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      // const response = await api.verifyPhoneOtp({ phone: phone.trim(), code: otp });
      setMode('signup');
    } catch (error) {
      const message = error instanceof ApiError ? error.message : 'Invalid code or verification failed.';
  ...
  const handleCreateAccount = async () => {
    ...
    try {
      ...
      const username_sanitized = username.trim().toLowerCase().replace(/[^a-z0-9_]/g, '');
      const signupPassword = authMethod === 'email' ? password.trim() : Crypto.randomUUID();
      
      console.log('[Auth] Attempting signup with username:', username_sanitized);
      ...
      if (error instanceof ApiError) {
        if (error.code === 409) {
          ...
          errorMessage = `Connection Error: Make sure the server is running at the API URL.\n\n${error.message}`;
        } else {
          errorMessage = error.message;
        }
